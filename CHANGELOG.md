...
### Adicionado (seleção por número nos menus de texto — UX pós-remoção do sendList)

- O hotfix anterior (seção abaixo) trocou listas tocáveis por texto simples, mas exigia digitar o nome completo da opção — atrito real pro cliente final. Agora **toda mensagem de opções gerada pelo bot é numerada** (`1. Corte`, `2. Barba`, ...) e o cliente responde só com o número; digitar o nome/data/horário continua funcionando igual antes.
- Arquitetura: `enviar_opcoes_em_texto()` (única função que já montava essas mensagens, usada nos 7 pontos do fluxo que apresentam opções — menu principal, serviços, horários, reagendamento) passou a numerar as linhas e gravar, no `contexto` do estado da conversa (Redis), o mapeamento `[índice] -> id da opção` (`CHAVE_OPCOES`) junto com o texto exibido (`CHAVE_OPCOES_TEXTO`). Uma única função central, `_resolver_selecao_numerica()`, chamada uma vez no topo de `processar_mensagem`, traduz uma resposta numérica pro id da opção do passo atual e segue o roteamento existente como se fosse um toque real — nenhum `if texto == "1"` foi espalhado pelos handlers. Como o mapeamento vive dentro do `contexto` de cada passo (que já é substituído a cada transição de estado), "1" no menu principal e "1" na lista de horários resolvem para ids diferentes automaticamente, sem lógica extra.
- Número fora do intervalo mostrado não avança o passo nem derruba o contexto: o cliente recebe a lista de opções de novo com o intervalo válido, e pode tentar de novo (por número ou por texto) sem perder o que já tinha escolhido antes daquela mensagem.
- **Correção crítica encontrada na revisão pré-deploy**: `_resolver_selecao_numerica` usava `str.isdigit()`, que também é `True` para caracteres como `²` (sobrescrito) ou `½` (fração) — mas `int()` não converte esses caracteres e levantava `ValueError` sem tratamento, derrubando o webhook com `500` (o mesmo tipo de incidente que motivou trocar o `enviar_lista`). Trocado para `str.isdecimal()`, o subconjunto que `int()` sempre converte com segurança.
- Efeito colateral corrigido de passagem: as sugestões de horário mostradas quando uma data digitada em texto livre já está ocupada (`_horario_texto_livre`) agora entram no mesmo passo `aguardando_slot` usado pelas demais listagens de horário (antes ficavam em `aguardando_horario_texto`, só reaproveitáveis redigitando a data por extenso) — passam a ser selecionáveis por número como qualquer outra lista de horários.
- Escopo: só as mensagens de texto com opções (antigo `enviar_lista`) ganharam numeração. Botões nativos (`enviar_botoes` — período, confirmação de cancelamento, atalhos pós-agendamento) não foram alterados: já são tocáveis e aceitam texto digitado, sem o problema que motivou essa mudança.
- Testes: 11 novos casos em `tests/test_conversa.py` cobrindo seleção numérica no menu principal, seleção fora do intervalo (com verificação de que o estado fica intacto), seleção de serviço por número e por texto lado a lado, fluxo completo menu → serviço → horário só por número (validando que o agendamento final bate com o horário do número escolhido), reagendamento por número, os dois ramos de "sugestões" (horário digitado em texto livre e reagendamento, ambos após um conflito), normalização de espaços/zeros à esquerda, listas com mais de 10 opções (dígitos duplos) e uma regressão específica para dígitos Unicode não-decimais (`²`, `½`) que quebrariam `_resolver_selecao_numerica` com `str.isdigit()`.
